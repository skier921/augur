...
@register_metric()
def pull_requests_first_time_opened(repo_group_id, repo_id=None, period='day', begin_date=None, end_date=None):
    """
    ...
    ?begin_date=2023-01-01&end_date=2023-06-01-01 for URL parameters
    """
    ...
    if repo_id:
        prNewContributor = s.sql.text("""
                SELECT
                    date_trunc(:period, new_date::DATE) as pr_date, --day, week, month, year 
                    COUNT(*),
                    repo_name
                FROM (
                    SELECT
                            pr_augur_contributor_id,
                            MIN(pull_requests.pr_created_at) AS new_date,
                            repo_name
                    FROM
                            pull_requests JOIN repo ON pull_requests.repo_id = repo.repo_id  
                    WHERE
                            pull_requests.repo_id = :repo_id
                            AND pull_requests.pr_created_at BETWEEN :begin_date AND :end_date
                    GROUP BY pr_augur_contributor_id, repo_name
                ) as abc
                GROUP BY pr_date, repo_name
                ORDER BY pr_date
            ) as abc
            GROUP BY issue_date, repo_name
            ORDER BY issue_date
        """)
        results = pd.read_sql(prNewContributor, engine, params={'repo_id': repo_id, 'period': period,
